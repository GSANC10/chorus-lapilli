...
# =========================== [ADD YOUR TESTS HERE] ===========================
    ...
    def test_button_upRight(self):
        ''' Check if clicking the top-right button adds an X. '''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        self.assertTileIs(tiles[2], self.SYMBOL_BLANK)
        tiles[2].click()
        self.assertTileIs(tiles[2], self.SYMBOL_X)
    def test_button_bottomRight(self):
        ''' Check if clicking the bottom-right button adds an X. '''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        self.assertTileIs(tiles[8], self.SYMBOL_BLANK)
        tiles[8].click()
        self.assertTileIs(tiles[8], self.SYMBOL_X)
    def test_prevent_moves_afterWin(self):
        '''Check that the game prevents moves after a win '''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        tiles[0].click()
        tiles[6].click()
        tiles[1].click()
